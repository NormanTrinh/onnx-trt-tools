...
class ImageBatcher:
    ...
    def __init__(self, input, shape, dtype, max_num_images=None, exact_batches=False, preprocessor="FR", shuffle_files=False):
        """
        ...
        :param preprocessor: Set the preprocessor to use, self-defined, depending on which network is being used.
    ...
    def preprocess_image(self, image_path):
        ...
        * EfficientDet: Resizes and pads the image to fit the input size.
        ...
        def resize_pad(image, pad_color = (0, 0, 0)):
            ...
            pad.paste(pad_color, [0, 0, self.width, self.height])   # get from model
            pad.paste(image)
            return pad, scale

        scale = None
        image = Image.open(image_path)      # path of image, read with (H, W, C)
        ...
        if self.preprocessor == "EfficientDet":
            ...
            image, scale = resize_pad(image, (0, 0, 0))
            image = np.asarray(image, dtype=self.dtype) # (H, W, C)
        ...
        elif self.preprocessor == "FR":
            # For vit face recognition
            image = image.resize((112, 112))
            image = np.array(image)     # already RGB format
